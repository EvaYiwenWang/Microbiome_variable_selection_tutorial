...
As all these above plots do not visualise the coefficients of the selected variables, we then apply a modified plotLoadings [@rohart2017mint] to display the coefficients. 
...
The interpretation of Figure \@ref(fig:loadclrCD) is the same as Figure \@ref(fig:loadcodaCD). From Figure \@ref(fig:loadclrCD), all the selected variables that more abundant in non-CD samples are assigned with positive coefficients. It means CLR-lasso seems more optimal than CoDA-lasso. Both *Roseburia* and *Peptostreptococcaceae* are selected by CoDA-lasso and CLR-lasso that are more abundant in non-CD group. Although both *Eggerthella* and *Dialister* are also selected by two methods, but with very different coefficient rank. 


### Trajectory plots

To visualise the change of variable coefficients and their ranks in the selection between different methods, we introduce trajectory plots.
...
In Figure \@ref(fig:loadcodaHFHS), two OTUs **1105860: Allobaculum** and **175272: S24-7** are more abundant in HFHS group but are assigned with positive coefficients. 
...
save_folder = '/Users/yiwenw5/Documents/GitHub/Methods_comparison/Methods_comparison/graphlan')
```
