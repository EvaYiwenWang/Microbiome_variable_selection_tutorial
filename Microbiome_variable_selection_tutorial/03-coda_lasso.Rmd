...
**lambda** is the penalisation parameter, the larger the value of **lambda** the less variables will be selected.

*Notes*: 

* Imputation of zeros: The user should provide a matrix **X** of positive values without zeros. Zeros should be previously imputed by the user. 
...
```{r}
lambdaRange_codalasso(y = y_Crohn, X = x_Crohn, lambdaSeq = seq(0.1, 0.2, 0.01))
```
...
```{r}
codalasso_Crohn <- coda_logistic_lasso(y = y_Crohn, X = x_Crohn, lambda = 0.15)
```

The same as the previous two methods, we also use a wrapper function called *coda_lasso_wrapper()* with the same input as *coda_logistic_lasso()*.

```{r}
Crohn.results_codalasso <- coda_lasso_wrapper(Y = y_Crohn, X = x_Crohn, 
                                              lambda = 0.15)
```
...
lambdaRange_codalasso(y = y_HFHSday1, X = x_HFHSday1, lambdaSeq = seq(1.3, 1.8, 0.01))
```
...
```{r}
HFHS.results_codalasso <- coda_lasso_wrapper(Y = y_HFHSday1, X = x_HFHSday1, lambda = 1.47) 
```
